    ...
    public class HotelServiceReservationService : IHotelServiceReservationService
    {
        ...
        public async Task<(bool Success, string Message)> MakeReservation(int userID, HotelServiceReservationCreateDTO request, List<string> roles)
        {
            ...
            var isConflicting = await _context.HotelServiceReservations.AnyAsync(res =>
                ...
                res.ServiceStatus!.ServiceStatusName != "Cancelled" &&
                res.ServiceStatus.ServiceStatusName != "Completed");
        ...
        public async Task<IEnumerable<HotelServiceReservationDTO>> GetUserReservations(int userID)
        {
            return await Task.Run(() =>
                _context.HotelServiceReservations
                    .Include(r => r.Service)
                    .Include(r => r.ServiceStatus)
                    .Include(r => r.CreatedByServiceReceptionist)
                        .ThenInclude(recep => recep.User)
                    .Where(r => r.UserID == userID)
                    .AsEnumerable()
                    .Select(r => new HotelServiceReservationDTO
                    {
                        ReservationID = r.ReservationID,
                        ServiceName = r.Service?.HeroTitle ?? "Unknown",
                        UserID = r.UserID,
                        ...
                        TimeSlot = r.TimeSlot,
                        ReservationStatusName = r.ServiceStatus?.ServiceStatusName ?? "Unknown",
        ...
        public async Task<string> CancelReservation(int reservationID, int userID, bool isAdminOrStaff = false)
        {
            ...
            reservation.ServiceStatusID = cancelledStatus.ReservationStatusID;
            await _context.SaveChangesAsync();
        ...
        {
            var reservation = await _context.HotelServiceReservations.Include(r => r.ServiceStatus).FirstOrDefaultAsync(r => r.ReservationID == reservationID);
        ...
        public async Task<string> UpdateReservationStatus(int reservationID, int statusID)
        {
            ...
            reservation.ServiceStatusID = statusID;
            await _context.SaveChangesAsync();
        ...
        public async Task<string> MarkReservationCompleted(int reservationID, int userID)
        {
            ...
            reservation.ServiceStatusID = completedStatus.ReservationStatusID;
            await _context.SaveChangesAsync();
